from models.conformer_model import ConformerBlock
import torch
...
class MaskDecoder3(nn.Module):
    def __init__(self, num_features, num_channel=64, out_channel=1):
        super(MaskDecoder3, self).__init__()
...
class MaskDecoder3_pf(nn.Module):
    def __init__(self, num_features, num_channel=64, out_channel=1):
        super(MaskDecoder3_pf, self).__init__()
        self.dense_block = DilatedDenseNet(depth=4, in_channels=num_channel)
        self.sub_pixel = SPConvTranspose2d(num_channel, num_channel, (1, 3), 2)
        self.conv_1 = nn.Conv2d(num_channel, out_channel, (1, 2))
        self.norm = nn.InstanceNorm2d(out_channel, affine=True)
...
class ComplexDecoder(nn.Module):
    def __init__(self, num_channel=64, out_channel=2):
        ...
        self.conv = nn.Conv2d(num_channel, out_channel, (1, 2))
...
class TSCNet(nn.Module):
    def __init__(self, num_channel=48, num_features=257):
        super(TSCNet, self).__init__()
        self.dense_encoder = DenseEncoder(in_channel=11, channels=num_channel)
        ...
        self.mask_decoder = MaskDecoder3(num_features, num_channel=num_channel, out_channel=2)
        self.complex_decoder = ComplexDecoder(num_channel=num_channel, out_channel=10)
    ...
    def forward(self, x):
        ...
        SPP, phi_S= self.mask_decoder(out_5)
        RTF = self.complex_decoder(out_5)        
        
        SPP= torch.sigmoid(SPP)     
        phi_S= torch.sigmoid(phi_S)
        RTF = torch.sigmoid(RTF)        
        
        return SPP, RTF, phi_S 


class TSCNet_PF(nn.Module):
    def __init__(self, num_channel=48, num_features=257):
        super(TSCNet_PF, self).__init__()
        ...
        self.mask_decoder3 = MaskDecoder3_pf(num_features, num_channel=num_channel, out_channel=3)
        
    def forward(self, x):
        x_in = x.unsqueeze(dim=1) #[B,T,F]->[B,1,T,F]
        ...
        xi,SPP,phi_S =self.mask_decoder3(out_5)

        xi = torch.sigmoid(xi)
        SPP= torch.sigmoid(SPP)     
        phi_S= torch.sigmoid(phi_S)
        
        return xi.squeeze(dim=1), SPP.squeeze(dim=1), phi_S.squeeze(dim=1)  
