...
class Trans_Ceps(nn.Module):
    def __init__(self):
        super(Trans_Ceps, self).__init__()



    def __call__(self, x0):
        Bsize,Fsize,Tsize = x0.shape
        # SPSD = torch.zeros([Bsize,Fsize,Tsize]).cuda()
        SPSD_ceps = torch.zeros([Bsize,Fsize,Tsize]).cuda()

        for ii in range(Tsize):
            x1 = torch.log(x0[:,:,ii]+1e-20)
            x2 = torch.real(torch.fft.ifft(torch.cat((x1,torch.flip(x1[:,1:-1], [1])), 1), 512))
            SPSD_ceps[:,:,ii] = x2[:,0:Fsize]
            
        return SPSD_ceps 

class Inverse_Trans_Ceps(nn.Module):
    def __init__(self):
        super(Inverse_Trans_Ceps, self).__init__()
    ...
    def __call__(self, x0):
        ...
        for ii in range(Tsize):               
            SPSD_fft = torch.real(torch.fft.fft(torch.cat((x0[:,:,ii],torch.flip(x0[:,:,ii][:,1:-1], [1])), 1), 512))
            SPSD_fft_half  = torch.exp(SPSD_fft[:,0:Fsize])
            SPSD[:,:,ii] = SPSD_fft_half #* 1.3346
        return SPSD 
